...
def _resample_with_value_line(timeframe, tf_values, ohlc):
    ...
    for tf in tf_values:
        ...
        ohlc[tf] = _normalize(ohlc, ohlc[tf])
    ...
    ohlc.avg = _normalize(ohlc, ohlc.avg)
    
    return ohlc

def _make_average_value_line(tf_values)-> pd.Series: 
...
def plot_value(symbols:list, timeframe:str, num_candles:int, num_charts:int, using_currency_index_data:bool=False, single_value_line=True):
    ''' Get ohlc data, overlay the value data and plot '''
...
symbols = ['UBERNYSE']
plot_value(symbols, '240 min', 1000, 20)
CORR_CON.close()
VAL_CON.close()